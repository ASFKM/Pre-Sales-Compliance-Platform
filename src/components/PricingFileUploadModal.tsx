...
import { X, FileUp, Loader2, CheckCircle2, CircleAlert, Sparkles, Activity } from "lucide-react";
...
interface QueuedFile {
  file: File;
  status: "queued" | "uploading" | "done" | "task_created" | "error";
...
export default function PricingFileUploadModal({
    ...
    try {
      ...
      setQueue((prev) =>
        prev.map((q) => {
          ...
          if (aiTask) {
            return { ...q, status: "task_created" as const };
          }
      ...
      // segundo plano (acompanhe na barra de tarefas do rodapé) - onDone() já atualiza o que dá
      // pra atualizar imediatamente (catálogo + contagem de extrações pendentes).
      onDone();
    } catch (e: any) {
  ...
  const statusLabel = (q: QueuedFile) => {
    ...
    if (q.status === "task_created") return "Tarefa criada — acompanhe o progresso na barra de tarefas, no rodapé";
    if (q.templateResult) {
      ...
      <div className="bg-white rounded-xl border border-slate-200 w-full max-w-xl overflow-hidden shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          <p className="text-xs text-slate-500">
            ...
            detecta sozinho: a planilha modelo importa direto; qualquer outro arquivo é analisado por IA (acompanhe o
            progresso na barra de tarefas do rodapé) e vira itens para você revisar antes de entrar no catálogo.
          </p>
            ...
            <div className="space-y-1.5 max-h-56 overflow-y-auto">
              {queue.map((q, idx) => (
                <div key={idx} className="flex items-center gap-2 text-xs bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-2">
                  ...
                  {q.status === "uploading" && <Loader2 size={13} className="animate-spin text-slate-400 shrink-0" />}
                  {q.status === "done" && <CheckCircle2 size={13} className="text-emerald-500 shrink-0" />}
                  {q.status === "task_created" && <Activity size={13} className="text-amber-500 shrink-0" />}
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            <button
              onClick={onClose}
              disabled={uploading}
              className="text-sm font-medium text-slate-600 border border-slate-300 rounded-lg px-3 py-2 hover:bg-slate-50 disabled:opacity-60"
            >
              Fechar
            </button>
          </div>
