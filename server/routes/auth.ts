...
/**
 * F1 (31/08/2026) - `ensurePasswordHashes` NAO voltou junto com o resto do login.
 *
 * Ela semeava "password123" em todo usuario sem hash, fora de producao. Isso fazia sentido
 * quando a coluna sempre esteve preenchida e o buraco era a excecao; hoje e o contrario: a
 * migration destrutiva de 30/08 apagou os 28 hashes, entao no primeiro `POST /login` ela
 * daria a MESMA senha conhecida a todos os 28 usuarios de uma vez.
 *
 * A decisao do dono e explicita: so o administrador master volta agora
 * (`scripts/f1-restaurar-administrador-master.ts`); os outros 27 ficam para depois. Quem esta
 * sem hash nao entra - `comparePasswords` recusa hash vazio, e a resposta e a mesma de uma senha
 * errada, sem contar a ninguem quais contas estao nesse estado.
 */
...
router.post("/login", loginRateLimiter, async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    await runWithTenant({ tenantId: user.tenant_id }, async () => {
      ...
      // A guarda continua valendo mesmo sem `ensurePasswordHashes`: o seed de desenvolvimento
      // (`prisma/seed.ts`) e os scripts de prova ainda gravam senhas conhecidas, e um banco de
      // desenvolvimento promovido a producao nao pode entrar com elas.
...
router.get("/me", async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    // F1 (31/08/2026) - buraco que o codigo anterior a Fase 13 tinha, e que nao vale a pena
    // restaurar junto com o resto.
    //
    // Esta rota resolve a sessao por conta propria, sem passar por `requireAuth`, e por isso
    // nunca via `must_change_password`. O gate existia so no requireAuth: quem tinha a troca
    // pendente batia em 403 em todas as rotas, MENOS nesta. E o `App.tsx` usa exatamente esta
    // rota para decidir, ao recarregar a pagina, se a pessoa ja esta autenticada - entao
    // recarregar a aba com a troca pendente entregava a casca do produto, com todas as outras
    // telas dando 403 sem explicacao.
    //
