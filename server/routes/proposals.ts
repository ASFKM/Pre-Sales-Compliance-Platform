...
router.post("/projects/:projectId/proposals/:type", requirePermission("proposal:generate"), async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    // 2. Perform variable replacement and write to filesystem (local uploads)
    const templateFile = path.join(process.cwd(), "uploads", "templates", "standard.docx");
    await generateDocxFromTemplate(templateFile, docxPath, templateData);

    // 3. Render real PDF via the PDF export adapter
    await generatePdfFromProposal(docxPath, pdfPath);
    ...
    logDebugMessage({
      ...
      message: `Generated real DOCX & PDF proposal matching variable requirements for project ${projectId}`,
      status: "SUCCESS",
    ...
    dbStore.addAuditLog({
      ...
      metadata: JSON.stringify({ type: proposalType, docx: proposal.docx_file_path })
    });
