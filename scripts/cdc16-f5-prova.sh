#!/usr/bin/env bash
# CDC 16 — Fase 5. Sobe um PreSales de verdade num banco dedicado e roda a ETAPA 2 contra ele.
#
...
PORTA="${PORTA:-3010}"
LOG="${LOG:-/tmp/cdc16-f5-servidor.log}"
...
if [ "${SO_SUBIR:-0}" = "1" ]; then
  ...
  while [ ! -f "${SINAL:-/tmp/cdc16-f5-parar}" ]; do sleep 2; done
  rm -f "${SINAL:-/tmp/cdc16-f5-parar}"
  exit 0
fi

BASE_URL="http://127.0.0.1:$PORTA" npx tsx "${SCRIPT_DA_PROVA:-scripts/cdc16-f5-provar-sla.ts}"
RESULTADO=$?
