#!/usr/bin/env bash
# CDC 16 — Fase 9. Sobe um PreSales de verdade num banco dedicado e roda a ETAPA 2 contra ele.
#
...
PORTA="${PORTA:-3019}"
LOG="${LOG:-/tmp/cdc16-f9-servidor.log}"
...
# `server.ts` avalia a bandeira de gestão de frota 30 s depois do boot, para o heartbeat COMPLETO.
# A prova precisa da licença em cache e liga essa bandeira por um instante para fazer um GET de
# leitura; esperar aqui garante que a avaliação do heartbeat completo já passou com a bandeira
# em `false`, e que nenhuma escrita deste servidor de prova chegue ao CMSaaS real.
echo "== esperando passar a janela do heartbeat completo (35 s)"
sleep 35
...
if [ "${SO_SUBIR:-0}" = "1" ]; then
  ...
  while [ ! -f "${SINAL:-/tmp/cdc16-f9-parar}" ]; do sleep 2; done
  rm -f "${SINAL:-/tmp/cdc16-f9-parar}"
  exit 0
fi

BASE_URL="http://127.0.0.1:$PORTA" npx tsx "${SCRIPT_DA_PROVA:-scripts/cdc16-f9-provar-fila.ts}"
RESULTADO=$?
